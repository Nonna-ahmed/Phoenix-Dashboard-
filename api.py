...
LATEST_AVAILABLE_DATE = CLIMATE_DF["date"].max().normalize()  # most recent date we have finalized weather data for
# NOTE: NASA POWER has a ~3-5 day processing lag, so this is NOT the same as
# the real calendar "today" — the API is honest about this via the date it returns.
...
def risk_map(date: date_type = Query(..., description="Date to evaluate, e.g. 2026-08-12")):
    ...
    Health/air-quality fields (live PM2.5) are only populated when `date` equals
    the LATEST available weather date in our dataset — not necessarily today's
    real calendar date, since NASA POWER has a processing lag. Older dates
    return null health fields rather than fabricating past air-quality."""
