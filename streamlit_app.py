...
is_today = pd.Timestamp(selected_date).normalize() == pd.Timestamp.now().normalize()
...
if not is_today:
    st.caption("ℹ️ Air-quality/health warnings are only available for **today's** date "
               "(live data source) — select today on the slider to see them.")
